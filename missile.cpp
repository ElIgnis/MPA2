...
Missile::Missile(char* filename, float x, float y, float w, int ownerID, string ownerName) :
	angular_velocity(0)
	, collision_X(0.f)
	, collision_Y(0.f)
	, ownername("")
...
float Missile::GetCollisionX(void)
{
	return collision_X;
}
float Missile::GetCollisionY(void)
{
	return collision_Y;
}

string Missile::GetOwnerName(void)
{
	return ownername;
}

float Missile::GetMissileDmg(void)
{
	return (damage + additional_damage);
}
void Missile::SetMissilePower(int level)
{
	additional_damage = level * 10;
}

bool Missile::GetSelfDamage(void)
{
	return selfDamage;
}